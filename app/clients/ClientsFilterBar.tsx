...
export default function ClientsFilterBar({ viewMode, setViewMode }: { viewMode: "list" | "grid"; setViewMode: (v: "list" | "grid") => void }) {
  ...
  const dropdowns = [
    { label: "Sector", open: sectorOpen, setOpen: setSectorOpen, options: ["All Sectors", "Banking", "Insurance", "Industrial", "Consumer Goods", "Pharma", "Energy"] },
    { label: "Region", open: regionOpen, setOpen: setRegionOpen, options: ["All Regions", "EU (Nordics)", "EU (Germany)", "EU (France)", "EU (UK/NL)", "EU (Switzerland)", "EU (Spain)", "MENA", "Asia", "Americas"] },
    { label: "Status", open: statusOpen, setOpen: setStatusOpen, options: ["All Statuses", "Active", "Needs Review", "Draft", "Closing Soon"] },
  ];
      ...
      <div className="flex items-center gap-2 flex-shrink-0 ml-4">
        {dropdowns.map((d) => (
          <div key={d.label} className="relative">
            ...
            {d.open && (
              <div className="absolute right-0 top-full mt-1 w-44 bg-white rounded-xl border border-[#ECEEF2] shadow-lg z-30 py-1 max-h-56 overflow-y-auto">
                {d.options.map((o) => (
                  <button
                    key={o}
